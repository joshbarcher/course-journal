// Ported from public/js/utils.js. escapeHtml is omitted — Svelte templates
// auto-escape {expr} interpolations, so it has no equivalent use here.
...
export const TYPE_ORDER: PageType[] = ['list', 'progress', 'progress-bars', 'notes', 'page'];
...
export function groupPagesByType(pages: Page[]): PageGroup[] {
	const map = new Map<PageType, Page[]>();
	for (const page of pages) {
		if (!map.has(page.type)) map.set(page.type, []);
		map.get(page.type)!.push(page);
	}
