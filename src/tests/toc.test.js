...
import { groupPagesByType, TYPE_ORDER } from '../../public/js/utils.js'

function page(type, title) {
    return { id: `id-${title}`, type, title }
}
...
describe('groupPagesByType', () => {
    ...
    it('attaches the correct label to each group', () => {
        ...
        const result = groupPagesByType(pages)
        assert.deepEqual(result.map(g => g.label), ['Lists', 'Progress', 'Notes', 'Pages'])
    })
    ...
    it('preserves page order within each group', () => {
        ...
        assert.deepEqual(group.pages.map(p => p.title), ['A', 'B', 'C'])
    })

    it('handles unknown types by using the type string as the label', () => {
        const pages = [{ id: '1', type: 'custom', title: 'X' }]
        const result = groupPagesByType(pages)
