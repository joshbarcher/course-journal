...
import { sortedItems, reorderItems } from '../../public/js/views/list.js'

function item(id, order, title = '') {
    return { id, order, title, subtasks: [] }
}
...
describe('sortedItems', () => {
    ...
    it('treats missing order as 0', () => {
        const page = { items: [{ id: 'x', title: '' }, item('a', 0)] }
        const result = sortedItems(page)
