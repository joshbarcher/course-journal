...
const { parseListItems, applyIndent, renderListItems } =
    await import('../../public/js/views/page-helpers.js')
...
describe('parseListItems', () => {
    ...
    test('parses multiple flat items', () => {
        assert.deepEqual(
            parseListItems(ul('<li>a</li><li>b</li>')),
            [{ depth: 0, html: 'a' }, { depth: 0, html: 'b' }],
        )
    })
    ...
    test('parses nested list into depth-tracked flat array', () => {
        assert.deepEqual(
            parseListItems(ul('<li>parent<ul><li>child</li></ul></li>')),
            [{ depth: 0, html: 'parent' }, { depth: 1, html: 'child' }],
        )
    })
    ...
    test('handles three levels of nesting', () => {
        assert.deepEqual(
            parseListItems(ul('<li>a<ul><li>b<ul><li>c</li></ul></li></ul></li>')),
